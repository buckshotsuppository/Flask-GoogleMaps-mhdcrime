...
class Map(object):
    ...
    def build_marker_dict(self, marker, icon=None):
        ...
        if len(marker) > 5:
            marker_dict["year"] = marker[5]
        if len(marker) > 5:
            marker_dict["marker_id"] = marker[6]
        if len(marker) > 6:
            marker_dict["reason"] = marker[7]
        if len(marker) > 7:
            marker_dict["location"] = marker[8]
        if len(marker) > 8:
            marker_dict["lat"] = marker[9]
        if len(marker) > 9:
            marker_dict["lon"] = marker[10]
        if len(marker) > 10:
            marker_dict["criminal_name"] = marker[11]
        return marker_dict
