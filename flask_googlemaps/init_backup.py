...
DEFAULT_CLUSTER_IMAGE_PATH = "static/assets/img/m"
...
class Map(object):
    ...
    def build_marker_dict(self, marker, icon=None):
        ...
        if len(marker) > 5:
            marker_dict["call_number"] = marker[5]
        if len(marker) > 6:
            marker_dict["reason"] = marker[6]
        if len(marker) > 7:
            marker_dict["action_taken"] = marker[7]
        if len(marker) > 8:
            marker_dict["date"] = marker[8]
        if len(marker) > 9:
            marker_dict["location"] = marker[9]
        if len(marker) > 10:
            marker_dict["lat"] = marker[10]
        if len(marker) > 11:
            marker_dict["lon"] = marker[11]
        if len(marker) > 12:
            marker_dict["units"] = marker[12]
        if len(marker) > 13:
            marker_dict["vehicle"] = marker[13]
        if len(marker) > 14:
            marker_dict["vehicle_towed"] = marker[14]
        if len(marker) > 15:
            marker_dict["criminal_name"] = marker[15]
        if len(marker) > 16:
            marker_dict["criminal_age"] = marker[16]
        if len(marker) > 17:
            marker_dict["criminal_address"] = marker[17]
        if len(marker) > 18:
            marker_dict["criminal_charges"] = marker[18]
        return marker_dict
